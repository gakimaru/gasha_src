...
//排他ロック（ライトロック）取得
void simple_shared_spin_lock::lock(const int spin_count)
{
...
bool simple_shared_spin_lock::try_lock()
{
	const int lock_counter = m_lockCounter.fetch_sub(SHARED_LOCK_COUNTER_UNLOCKED);//カウンタを更新
	if (lock_counter == SHARED_LOCK_COUNTER_UNLOCKED)
		return true;//ロック取得成功
...
//共有ロック（リードロック）取得
void simple_shared_spin_lock::lock_shared(const int spin_count)
{
	int spin_count_now = spin_count;
	while (1)
	{
...
//共有ロック（リードロック）取得を試行
bool simple_shared_spin_lock::try_lock_shared()
{
	const int lock_counter = m_lockCounter.fetch_sub(1);//カウンタを更新
	if (lock_counter >= 0)
		return true;//ロック取得成功
